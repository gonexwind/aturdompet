...
import 'package:aturdompet/screens/outcome/minggu/view_outcome.dart';
import 'package:flutter/material.dart';
import 'package:aturdompet/widgets/custom_icon_decoration.dart';
import 'package:hive_flutter/hive_flutter.dart';
import 'package:intl/intl.dart';

class OutcomeListMinggu extends StatefulWidget {
  @override
  _OutcomeListMingguState createState() => _OutcomeListMingguState();
}

class _OutcomeListMingguState extends State<OutcomeListMinggu> {
  @override
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: OutcomeMingguViewModel.openBox(),
  ...
  Widget _outcomeItem() {
    ...
    return WatchBoxBuilder(
      box: OutcomeMingguViewModel.getall(),
      builder: (context, outcomeBox) {
        return ListView.builder(
          itemCount: outcomeBox.length,
          padding: const EdgeInsets.all(0),
          itemBuilder: (context, index) {
            final outcome = OutcomeMingguViewModel.getAt(index);
  ...
  Widget _displayItem(int index, OutcomeMinggu outcome, BuildContext context) {
    return Expanded(
      child: Padding(

...
import 'package:aturdompet/screens/outcome/hari/view_outcome.dart';
import 'package:flutter/material.dart';
import 'package:aturdompet/widgets/custom_icon_decoration.dart';
import 'package:hive_flutter/hive_flutter.dart';
import 'package:intl/intl.dart';

class OutcomeListHari extends StatefulWidget {
  @override
  _OutcomeListHariState createState() => _OutcomeListHariState();
}

class _OutcomeListHariState extends State<OutcomeListHari> {
  @override
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: OutcomeHariViewModel.openBox(),
  ...
  Widget _outcomeItem() {
    ...
    return WatchBoxBuilder(
      box: OutcomeHariViewModel.getall(),
      builder: (context, outcomeBox) {
        return ListView.builder(
          itemCount: outcomeBox.length,
          padding: const EdgeInsets.all(0),
          itemBuilder: (context, index) {
            final outcome = OutcomeHariViewModel.getAt(index);
  ...
  Widget _displayItem(int index, OutcomeHari outcome, BuildContext context) {
    return Expanded(
      child: Padding(

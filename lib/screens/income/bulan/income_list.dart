...
import 'package:aturdompet/screens/income/bulan/view_income.dart';
import 'package:flutter/material.dart';
import 'package:aturdompet/widgets/custom_icon_decoration.dart';
import 'package:hive_flutter/hive_flutter.dart';
import 'package:intl/intl.dart';

class IncomeListBulan extends StatefulWidget {
  @override
  _IncomeListBulanState createState() => _IncomeListBulanState();
}

class _IncomeListBulanState extends State<IncomeListBulan> {
  @override
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: IncomeBulanViewModel.openBox(),
  ...
  Widget _incomeItem() {
    ...
    return WatchBoxBuilder(
      box: IncomeBulanViewModel.getall(),
      builder: (context, incomeBox) {
        return ListView.builder(
          itemCount: incomeBox.length,
          padding: const EdgeInsets.all(0),
          itemBuilder: (context, index) {
            final income = IncomeBulanViewModel.getAt(index);
  ...
  Widget _displayItem(int index, IncomeBulan income, BuildContext context) {
    return Expanded(
      child: Padding(

...
import 'package:aturdompet/screens/income/hari/view_income.dart';
import 'package:flutter/material.dart';
import 'package:aturdompet/widgets/custom_icon_decoration.dart';
import 'package:hive_flutter/hive_flutter.dart';
import 'package:intl/intl.dart';

class IncomeListHari extends StatefulWidget {
  @override
  _IncomeListHariState createState() => _IncomeListHariState();
}

class _IncomeListHariState extends State<IncomeListHari> {
  @override
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: IncomeHariViewModel.openBox(),
  ...
  Widget _incomeItem() {
    ...
    return WatchBoxBuilder(
      box: IncomeHariViewModel.getall(),
      builder: (context, incomeBox) {
        return ListView.builder(
          itemCount: incomeBox.length,
          padding: const EdgeInsets.all(0),
          itemBuilder: (context, index) {
            final income = IncomeHariViewModel.getAt(index);
  ...
  Widget _displayItem(int index, IncomeHari income, BuildContext context) {
    return Expanded(
      child: Padding(
